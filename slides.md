...
| 符号 | 含义 | 解释 |
| -- | -- | -- |
| S<sub>team</sub> | 团队熵值 | 表示系统复杂性、组织混乱度 |
| C | 沟通链路数量 | 与团队人数相关 |
| L | 决策层级 | 层级越深，决策路径越长，复杂度呈非线性上升，例如：L<sup>1.5</sup> |
| D | 技术复杂度 | 如代码量、模块数、耦合度等 |
| T（≥ 1） | 工具减熵因子 | 如自动化测试覆盖率、CI/CD、文档完备性等 |
| P（≥ 1） | 开发模式成熟度 | 敏捷、DevOps、持续反馈等组织机制带来的有序性，不同模式给予不同权重，默认为 1 |
| k | 行业经验系数 | 行业内单位规模下的经验性调整系数，默认为 1 |
</v-click>
<!--
